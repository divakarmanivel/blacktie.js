...
function createCustomAlert(txt) {
	...
	if (modals.length != 0 && sessionStorage.getItem(txt) != "" && sessionStorage.getItem(txt) != null) {
		sessionStorage.setItem(txt, false);
	}
	...
	if ((sessionStorage.getItem(txt) != "") && (sessionStorage.getItem(txt) != null)) {
		var val = sessionStorage.getItem(txt);
		msg.innerHTML = val;
	} else {
		msg.innerHTML = txt;
	}
...
function createCustomConfirm(txt) {
	...
	mok.onclick = function () {
		sessionStorage.removeItem(txt);
		if (sessionStorage.getItem(txt) != "" && sessionStorage.getItem(txt) != null) {
			sessionStorage.setItem(txt, true);
		}
		removeCustomAlert(modalId);
		return true;
	};
